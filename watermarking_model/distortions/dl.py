...
def _get_phone_assets(target_sr: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    noise = (
        tf_resample(noise_raw, noise_sr, target_sr) if noise_sr != target_sr else noise_raw
    )
...
class distortion(nn.Module):
    def __init__(self, process_config):
        ...
        self.band_lowpass_2k = julius.LowPassFilter(2000 / self.sample_rate).to(
            self.device
        )
        self.band_lowpass_4k = julius.LowPassFilter(4000 / self.sample_rate).to(
            self.device
        )
    ...
    def low_pass_2k(self, y):
        return self.band_lowpass_2k(y)

    def low_pass_4k(self, y):
        return self.band_lowpass_4k(y)
    ...
    def benign_phone_distortion(self, x: torch.Tensor, ratio, sample_rate: int = 16000):
        ...
        # Apply RIR
        # mode="full" truncated keeps the room response causal; "same" centres it
        # and puts reverberation before the sound that caused it.
        rir_applied = fftconvolve(x, rir, mode="full")[..., : x.shape[-1]]
        snr_db = torch.randint(20, 26, (1,), device=x.device)
        bg_added = add_noise(rir_applied, noise, snr_db)

        y_d = julius.bandpass_filter(
            bg_added,
            cutoff_low=300 / sample_rate,
            cutoff_high=3400 / sample_rate,
        )
        y_d = y_d.unsqueeze(1)
        return y_d
