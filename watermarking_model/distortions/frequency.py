...
from librosa.util import pad_center as _pad_center, tiny


def pad_center(data, size):
    """librosa >=0.10 made `size` keyword-only; 0.9.x took it positionally."""
    try:
        return _pad_center(data, size=size)
    except TypeError:
        return _pad_center(data, size)
